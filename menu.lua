...
function createFreeplayMenu()
	...
	runHaxeCode([[
	    game.getLuaObject('diffText').camera = getVar('freeCam');
	    game.getLuaObject('diffText').scrollFactor.set(1, 1);
	]])
	addLuaText('diffText')
...
function onCustomSubstateCreate(name)
	...
	for v, i in ipairs(list) do
		...
		runHaxeCode([[
		    game.getLuaObject('songText]]..v..[[_]]..ii..[[').camera = getVar('freeCam');
		    game.getLuaObject('songText]]..v..[[_]]..ii..[[').scrollFactor.set(1, 1);
		]])
	...
	runTimer('allow', 0.65)
	onTimerCompleted = function(tag) if tag == 'allow' then allowInput = true end end
    end
	...
	for i = 0, 1 do
		...
		game.getLuaObject('brochure]]..i..[[').animation.finishCallback = (t) -> {
			...
			game.callOnLuas('setDataFromSave', ['StoryMenuState', 'isFrenzy', getVar('selectingFrenzy')]);
                        game.callOnLuas('closeCustomSubstate', ['Brochure Menu']);
                        game.callOnLuas('runTimer', ['openStory', 1]);
		    }
		}
	    ]])
	end
        onTimerCompleted = function(tag) if tag == 'openStory' then openCustomSubstate('Story Menu') end end
    ...
    if tag == 'Story Menu' then
        setVar('allowInput', true)
        setProperty('camOther.zoom', 0.91)
        setProperty('camOther.scroll.y', 20)

        --if not getDataFromSave('StoryMenuState', 'isFrenzy') then
            loadClassic()
        --else
            --loadFrenzy()
        --end
...
function loadClassic()
    makeLuaSprite('storybg', 'story/bg_og', -84, -30)
    setObjectCamera('storybg', 'other')
    addLuaSprite('storybg', true)

    makeAnimatedLuaSprite('tutorial', 'story/buildings/tutorial', 567, 109)
    addAnimationByPrefix('tutorial', 'selected', 'tutorialselected', 24, false)
    addAnimationByPrefix('tutorial', 'nonselect', 'tutorialnonselected', 24, false)
    setObjectCamera('tutorial', 'other')
    addLuaSprite('tutorial')
end

function get_weekData()
    return {
        {'Milk-Tea'},
